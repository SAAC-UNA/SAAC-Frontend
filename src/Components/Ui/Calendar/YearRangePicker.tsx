/**
 * YearRangePicker - Selector de rango de anios.
 *
 * Usa una sola entrada para seleccionar inicio y fin de un periodo anual.
 */
...
const toYearString = (year: number): string => String(year);

export const YearRangePicker: React.FC<YearRangePickerProps> = ({
  label,
  value,
  placeholder = 'Selecciona un rango de años...',
  disabled = false,
  error,
  helperText,
  required = false,
  ...
  const selectedFrom = toYearNumber(value?.from);
  const selectedTo = toYearNumber(value?.to);
  const hasValue = Boolean(selectedFrom && selectedTo);

  const [pageStartYear, setPageStartYear] = useState(selectedFrom ?? resolvedMinYear);
  ...
  useEffect(() => {
    if (selectedFrom) {
      setPageStartYear(Math.max(resolvedMinYear, selectedFrom));
    }
  }, [resolvedMinYear, selectedFrom]);
  ...
  const previewStart = selectedFrom && !selectedTo && hoverYear
    ? Math.min(selectedFrom, hoverYear)
    : selectedFrom;
  const previewEnd = selectedFrom && !selectedTo && hoverYear
    ? Math.max(selectedFrom, hoverYear)
    : selectedTo;

  const displayText = selectedFrom && selectedTo
  ...
  const handleSelectYear = (year: number) => {
    if (!selectedFrom || selectedTo) {
      onChange?.({ from: toYearString(year), to: undefined });
      return;
    }

    const from = Math.min(selectedFrom, year);
    const to = Math.max(selectedFrom, year);
    onChange?.({ from: toYearString(from), to: toYearString(to) });
    setHoverYear(null);
    setShowPicker(false);
  };
    ...
    <>
      ...
      <div className="grid grid-cols-4 gap-1 mb-3">
        {visibleYears.map((year) => {
          const isStart = year === previewStart;
          const isEnd = year === previewEnd;
          const isInRange = Boolean(previewStart && previewEnd && year > previewStart && year < previewEnd);
          ...
          return (
            <button
              ...
              type="button"
              onMouseEnter={() => {
                if (selectedFrom && !selectedTo) setHoverYear(year);
              }}
              ...
              className={cn(
                ...
                isInRange && 'bg-info-light/50 text-negro-una',
                isCurrent && !isSelected && !isInRange && 'bg-error-light text-error border border-error-ring',
                !isSelected && !isInRange && !isCurrent && 'hover:bg-gris-una/10 text-negro-una',
              )}
            >
              {year}
            </button>
    ...
    <div className="space-y-2" ref={containerRef}>
      <div className="relative">
        <button
          ...
          aria-label={label || 'Selector de rango de años'}
        >
          <span className={cn(
            'text-left flex-1',
          ...
          <div className="flex items-center gap-1">
            {hasValue && !disabled && (
              <div
                ...
                aria-label="Limpiar rango de años"
              >
