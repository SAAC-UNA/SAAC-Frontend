...
const CreateImprovementCommitment: React.FC = () => {
  ...
  const [formData, setFormData] = useState<CompromisoFormData>({
    ciclo_acreditacion_id: locationState.cicloId
      ? parseInt(locationState.cicloId)
      : null,
    proceso_id: locationState.procesoId
      ? parseInt(locationState.procesoId)
      : undefined,
    descripcion: "",
  ...
  useEffect(() => {
    ...
    if (isFlexible) {
      improvementCommitmentService
        .obtenerCompromisoElementosPorProceso(procesoId)
        .then((commitments) => {
      ...
      improvementCommitmentService
        .obtenerCompromisoPorProceso(procesoId)
        .then((commitment) => {
          ...
          const criteriosSeleccionados: CriterioSeleccionado[] = (
            ...
            const evidenciaIds: number[] = (sel.evidencias ?? []).map(
              (e: any) => e.evidencia_id,
            );
            const matchingAssignments = assignedEvidences.filter((ae) =>
              evidenciaIds.includes(ae.evidencia_id),
            );
            const uniqueUsers = [
              ...new Set(
                matchingAssignments.map((ae: any) => ae.usuario_id as number),
              ),
            ];
            return {
              ...
              comentario:
                matchingAssignments[0]?.pivot?.comentario ??
                matchingAssignments[0]?.comentario ??
                "",
    ...
    switch (step) {
      case 1:
        if (!formData.ciclo_acreditacion_id) {
          newErrors.ciclo_acreditacion_id =
  ...
  const handleNext = () => {
    ...
    if (!errs) {
      ...
      showToast({
        type: "error",
        title: firstMsg || "Por favor, complete todos los campos obligatorios",
      });
    }
  };
