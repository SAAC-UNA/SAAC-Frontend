...
describe("evaluateAccess", () => {
  ...
  describe("requireRoles", () => {
    ...
    it("devuelve true cuando el usuario tiene al menos uno de los roles", () => {
      expect(
        evaluateAccess(
          { roles: ["Docente"] },
          { requireRoles: ["Administrador", "Docente"] },
        ),
      ).toBe(true);
    });
    ...
    it("devuelve true cuando requireRoles está vacío", () => {
      expect(evaluateAccess({ roles: ["Docente"] }, { requireRoles: [] })).toBe(
        true,
      );
    });
  });
  ...
  describe("requireAnyPermissions", () => {
    ...
    it("devuelve true cuando requireAnyPermissions está vacío", () => {
      expect(
        evaluateAccess({ permissions: [] }, { requireAnyPermissions: [] }),
      ).toBe(true);
    });
  });
  ...
  describe("contexto vacío", () => {
    it("devuelve false con roles vacíos y regla de rol", () => {
      expect(evaluateAccess({}, { requireRoles: ["Administrador"] })).toBe(
        false,
      );
    });
    ...
    it("devuelve false con permisos vacíos y regla de permiso", () => {
      expect(
        evaluateAccess({}, { requireAnyPermissions: ["usuarios.view"] }),
      ).toBe(false);
    });
  });
});
...
describe("getUserRoleNames", () => {
  it("extrae los nombres de los roles", () => {
    const roles = [{ name: "Administrador" }, { name: "Docente" }];
    expect(getUserRoleNames(roles)).toEqual(["Administrador", "Docente"]);
  });
...
describe("getUserPermissionNames", () => {
  it("extrae los nombres de los permisos", () => {
    ...
    expect(getUserPermissionNames(permissions)).toEqual([
      "usuarios.view",
      "roles.view",
    ]);
  });
